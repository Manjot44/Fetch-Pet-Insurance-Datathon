...
cv_lasso <- cv.glmnet(x_train, y_train, family = "poisson", offset = offset_train, alpha = 0)
...
lasso_model <- glmnet(x_train, y_train, family = "poisson", offset = offset_train, alpha = 0, lambda = best_lambda)
...
lasso_model_cat2 <- cv.glmnet(predictors_cat2, response_cat2, family = "poisson", alpha = 0)
...
lasso_model_summary <- glmnet(predictors_cat2, response_cat2, family = "poisson", alpha = 0, lambda = best_lambda_cat2)
...
vars_to_remove_cat3 <- c("Potential.For.Weight.Gain", "Drooling.Potential", "Kid.Friendly",
                         ...
                         "cat1", "cat2", "claim_rate_cat1", "claim_rate_cat2", "claim_rate_cat3", "claim_count")
...
test_data_prem_cleaned_cat3 <- test_data[, !names(test_data) %in% vars_to_remove_cat3]
...
# Define the response variable and predictor matrix
response_cat3 <- train_data_cleaned_cat3$cat3
predictors_cat3 <- model.matrix(cat3 ~ . - 1, data = train_data_cleaned_cat3)  # Exclude intercept

# Fit the Lasso model on the cleaned training data with cross-validation to find the best lambda
lasso_model_cat3 <- cv.glmnet(predictors_cat3, response_cat3, family = "poisson", alpha = 0)

# Best lambda from cross-validation
best_lambda_cat3 <- lasso_model_cat3$lambda.min

# Make predictions on the test data
test_predictors_cat3 <- model.matrix(cat3 ~ . - 1, data = test_data_prem_cleaned_cat3)
predicted_frequencies_prem_cat3 <- predict(lasso_model_cat3, newx = test_predictors_cat3, s = best_lambda_cat3, type = "response")

# Print the first few predictions for Cat3
head(predicted_frequencies_prem_cat3)

# Summary of the Lasso model (best lambda)
lasso_model_summary_cat3 <- glmnet(predictors_cat3, response_cat3, family = "poisson", alpha = 0, lambda = best_lambda_cat3)
print(summary(lasso_model_summary_cat3))
