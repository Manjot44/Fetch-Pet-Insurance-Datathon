...
# Run up to line 420 of 'Adding_data2 (new)' sheet

severity_data <- severity_data %>%
  filter(nb_breed_trait != " ", !is.na(nb_breed_trait))

# remove bull (2), pinscher (10), setter(4), white fluffy (13)

### Cleaning: remove the 7 rows with " " or NA as the nb_breed_trait

# Convert nb_breed_trait to binary columns
binary_columns <- model.matrix(~ nb_breed_trait - 1, data = severity_data)

# Combine the binary columns with the original data (excluding the original nb_breed_trait)
severity_data <- cbind(severity_data, binary_columns)

# Optionally, remove the original nb_breed_trait column
severity_data <- severity_data %>% select(-nb_breed_trait)

# Remove "nb_breed_trait" from column names
colnames(severity_data) <- gsub("nb_breed_trait", "", colnames(severity_data))

# Remove specific columns from severity_data
severity_data <- severity_data %>%
  select(-bull, -pinscher, -setter, -`white fluffy`, -hound,
         -mastiff, -pointer, -`shepherd type`, - sighthound, - terrier)


# Define the binary breed traits
binary_vars <- c("brachycephalic", "collie related", 
                 "cross", "retriever",
                 "spaniel", "spitz related", "teckel", 
                 "unknown", "water")

# Rename 'water dog' to 'water' in the severity_data dataframe
colnames(severity_data)[colnames(severity_data) == "water dog"] <- "water"
...
# Subset severity_data with selected binary breed traits
severity_data_subset <- severity_data[, binary_vars]

# Convert logical binary columns (if needed) to numeric
severity_data_subset <- severity_data_subset %>%
  mutate(across(.cols = all_of(binary_vars), ~ as.numeric(.)))

# PCA analysis using only binary breed traits
pca_data <- severity_data_subset  # Select only binary traits (no response variable)

# Perform PCA
data.pca <- princomp(pca_data, cor = TRUE)

# Extract loadings matrix (eigenvectors)
loadings_matrix <- data.pca$loadings[, 1:9]

# Visualize eigenvalues
fviz_eig(data.pca, addlabels = TRUE)

# Modify the plot to adjust the label positions to the left of the arrows
g11 <- fviz_pca_var(data.pca, col.var = "black", label = "var", repel = FALSE) +
  labs(title = "PCA Breed Traits") +
  theme_bw() +
  theme_set(theme_bw()) +
  theme_update(plot.title = element_text(hjust = 0.5, size = 10, face = 'bold')) +
  theme_update(text = element_text(size = 10)) +
...
# Save the plot
ggsave(filename = "g11.png", height = 8, width = 13, units = 'cm', dpi = 1080, plot = g11)
...
severity_data <- severity_data[, c("nb_breed_type", "earned_units", "claim_count", "claim_paid")]

# Step 2: Remove rows where nb_breed_type is NA
severity_data <- severity_data %>%
  filter(!is.na(nb_breed_type))

# Step 3: Convert nb_breed_type to binary columns
binary_columns <- model.matrix(~ nb_breed_type - 1, data = severity_data)

# Step 4: Combine binary columns with original data (excluding nb_breed_type)
severity_data <- cbind(severity_data, binary_columns)

# Step 5: Optionally, remove the original nb_breed_type column
severity_data <- severity_data %>% select(-nb_breed_type)

# Step 6: Remove "nb_breed_type" from column names
colnames(severity_data) <- gsub("nb_breed_type", "", colnames(severity_data))

# Define the binary breed traits
binary_vars <- c("unnamed cross", "cross", "designerbreed", "purebred")

# Step 7: Subset severity_data with selected binary breed traits
severity_data_subset <- severity_data[, binary_vars]

# Step 8: Convert logical binary columns (if needed) to numeric
severity_data_subset <- severity_data_subset %>%
  mutate(across(.cols = all_of(binary_vars), ~ as.numeric(.)))

# Step 9: PCA analysis using only binary breed traits
pca_data_scaled <- scale(severity_data_subset)  # Scale data
data.pca <- prcomp(pca_data_scaled, center = TRUE, scale. = TRUE)  # Perform PCA

# Step 10: Extract loadings matrix (eigenvectors)
loadings_matrix <- data.pca$rotation[, 1:2]  # First 2 components

# Step 11: Visualize eigenvalues
fviz_eig(data.pca, addlabels = TRUE)
...
# Step 12: Plot the PCA results with arrows and labels, without dots or the circle
g12 <- fviz_pca_var(data.pca, col.var = "black", geom = "arrow") +  # Keep only arrows, remove points
  geom_text_repel(aes(x = data.pca$rotation[, 1], 
                      y = data.pca$rotation[, 2], 
                      label = rownames(data.pca$rotation)),
                  size = 4, color = "black") +    # Adjust text size
  labs(title = "PCA Breed Type") + 
  theme_bw() + 
  theme_set(theme_bw()) + 
  theme_update(plot.title = element_text(hjust = 0.5, size = 10, face = 'bold')) + 
  theme_update(text = element_text(size = 10)) + 
...
# Save the plot
ggsave(filename = "g12.png", height = 8, width = 13, units = 'cm', dpi = 1080, plot = g12)
